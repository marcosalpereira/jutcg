...
public class NextValueForType {
	...
	public String next(Type type) {
		...
		if (!type.isPrimitive()) {
            ...
            final String valueTemplate = Config
                .getString(String.format("valueTemplateFor.%s", name));
			if (valueTemplate != null) {
			    if (valueTemplate.contains(VALUE_TEMPLATE_ID_VAR)) {
			        return valueTemplate.replaceAll(VALUE_TEMPLATE_ID_VAR, String.valueOf(nextId(type)));
			    } else {
	...
	private String collectionInstance(Type type) {
		...
		if (generic == null) {
			...
			if (type.isSet()) {
			    GlobalFlags.setNewHashSet(true);
				return String.format("newHashSet(%s)", next(generic[0]));
			} else if (type.isMap()) {
			    GlobalFlags.setNewHashMapUsed(true);
